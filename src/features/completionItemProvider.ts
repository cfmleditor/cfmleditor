        ...
        if (docIsCfmFile) {
            if (getValidScopesPrefixPattern([Scope.Variables], true).test(docPrefix)) {
                ...
                allTemplateFunctions.filter((func: UserFunction) => {
                    ...
                    const newProposal: CompletionItem | undefined = func.name ? createNewProposal(
                ...
                if (docIsCfcFile && !varScope && equalsIgnoreCase(varName, "super")) {
                    ...
                    while (currComponent) {
                        currComponent.functions.filter((_func: UserFunction, funcKey: string) => {
                            ...
                            const newProposal: CompletionItem | undefined = func.name && currComponent ? createNewProposal(
                            ...
                            if (initialFoundComp) {
                                ...
                                while (foundComponent) {
                                    ...
                                    foundComponent.functions.filter((func: UserFunction, funcKey: string) => {
                                        return func.access ? currentWordMatches(funcKey)
                                            && validFunctionAccess.has(func.access)
                                            && !addedFunctions.has(funcKey) : false;
                                    }).forEach((func: UserFunction, funcKey: string) => {
                                        const newProposal: CompletionItem | undefined = func.name && foundComponent ? createNewProposal(
    ...
    if (tagPrefixMatch) {
        ...
        for (const tagName in globalTags) {
            if (state.currentWordMatches(tagName)) {
                ...
                const thisGlobalTagCompletion: CompletionItem | undefined = globalTag.name ? createNewProposal(
                    globalTag.name,
